...
function writeWavFile(filename: string, audioData: Buffer, sampleRate: number = 16000, channels: number = 1, bitsPerSample: number = 16) {
...
class CallSession {
    ...
    private resampleAudio(inputData: Int16Array, inputRate: number, outputRate: number): Int16Array {
        ...
        console.log(`[RESAMPLE] Resampled from ${inputRate}Hz to ${outputRate}Hz: ${inputData.length} -> ${output.length} samples`);
        return output;
    }
    ...
    private async streamTts(text: string) {
        ...
        const ttsOutputDir = path.join(path.dirname(new URL(import.meta.url).pathname), '..', 'tts_output');
        ...
        const TTS_INPUT_SAMPLE_RATE = 24000;
        const TARGET_SAMPLE_RATE = 16000;
        
        // ✅ Buffer accumulator - batch frames before sending
        let audioBuffer: Int16Array = new Int16Array(0);
        const BATCH_SIZE_MS = 100;  // Accumulate 100ms of audio before sending
        const BATCH_SIZE_SAMPLES = (TARGET_SAMPLE_RATE * BATCH_SIZE_MS) / 1000;
    
        console.log(`[SESSION-${this.sessionId}] Processing TTS stream (batching every ${BATCH_SIZE_MS}ms / ${BATCH_SIZE_SAMPLES} samples)`);
        ...
        for await (const ev of ttsStream) {
            if (ev === ttsCore.SynthesizeStream.END_OF_STREAM) {
                ...
                if (audioBuffer.length > 0) {
                    console.log(`[SESSION-${this.sessionId}] Sending final audio batch: ${audioBuffer.length} samples`);
                    const pcm = Buffer.from(audioBuffer.buffer, audioBuffer.byteOffset, audioBuffer.byteLength);
                    audioChunks.push(pcm);
                    totalAudioBytes += pcm.length;
                    
                    if (this.ws.readyState === WebSocket.OPEN) {
                        this.ws.send(pcm, { binary: true });
                        console.log(`[SESSION-${this.sessionId}] Final audio batch sent (${pcm.length} bytes)`);
                    }
            ...
            // ✅ Get TTS audio at 24kHz and resample to 16kHz
            const audioData24k = frame.data as Int16Array;
            const audioData16k = this.resampleAudio(audioData24k, TTS_INPUT_SAMPLE_RATE, TARGET_SAMPLE_RATE);
    
            // ✅ Accumulate into buffer
            const combined = new Int16Array(audioBuffer.length + audioData16k.length);
            combined.set(audioBuffer);
            combined.set(audioData16k, audioBuffer.length);
            audioBuffer = combined;
    
            console.log(`[SESSION-${this.sessionId}] Buffered audio: ${audioBuffer.length} samples (need ${BATCH_SIZE_SAMPLES} for batch)`);
    
            // ✅ Send when batch is ready
            if (audioBuffer.length >= BATCH_SIZE_SAMPLES) {
                // Send complete batches, keep remainder
                const completeBatches = Math.floor(audioBuffer.length / BATCH_SIZE_SAMPLES);
                const batchSamples = completeBatches * BATCH_SIZE_SAMPLES;
                
                const batchToSend = audioBuffer.slice(0, batchSamples);
                audioBuffer = audioBuffer.slice(batchSamples);
    
                const pcm = Buffer.from(batchToSend.buffer, batchToSend.byteOffset, batchToSend.byteLength);
                console.log(`[SESSION-${this.sessionId}] Sending audio batch: ${batchToSend.length} samples (${pcm.length} bytes)`);
                ...
                if (this.ws.readyState === WebSocket.OPEN) {
                    this.ws.send(pcm, { binary: true });
                    console.log(`[SESSION-${this.sessionId}] Audio batch sent successfully`);
                } else {
                    console.log(`[SESSION-${this.sessionId}] WebSocket not open`);
                }
            }
        }
        ...
        if (audioChunks.length > 0) {
            ...
            const sanitizedText = text.substring(0, 50).replace(/[^a-zA-Z0-9\s]/g, '').replace(/\s+/g, '_');
            const filename = `tts_${this.sessionId}_${timestamp}_${sanitizedText}.wav`;
            const filepath = path.join(ttsOutputDir, filename);
    
            try {
                writeWavFile(filepath, combinedAudio, TARGET_SAMPLE_RATE, 1, 16);
                
                const durationSecs = (combinedAudio.length / (TARGET_SAMPLE_RATE * 2)).toFixed(2);
                console.log(`[SESSION-${this.sessionId}] TTS audio saved: ${filepath} (${combinedAudio.length} bytes, ${durationSecs}s @ 16kHz)`);
            } catch (error) {
                console.error(`[SESSION-${this.sessionId}] Error saving TTS audio:`, error);
            }
        }
    ...
    // Add this method to CallSession class for testing WAV file creation
    private async testTtsWavSaving() {
        console.log(`[SESSION-${this.sessionId}] Testing TTS WAV file saving...`);

        const testText = "Hello, this is a test of the TTS WAV file saving functionality.";
        await this.streamTts(testText);

        console.log(`[SESSION-${this.sessionId}] TTS WAV test completed. Check the tts_output folder.`);
    }
