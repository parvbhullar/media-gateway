# SuperSip WebSocket Interface (RWI)

RWI is a JSON-over-WebSocket control plane for SuperSip call orchestration. It follows Asterisk AMI's action/event model, adapted to JSON over WebSocket for modern contact center workflows.

## 1. Overview

RWI provides:
- **Command channel**: client → SuperSip
- **Event channel**: SuperSip → client
- **Optional media channel**: PCM binary frames (via separate media WebSocket)

RWI does not replace SIP signaling. It controls call behavior through SuperSip internal `CallController` and `SessionAction` abstractions.

## 2. Architecture

RWI is implemented as a built-in SuperSip module with core components:
...
```
1. SIP INVITE → SuperSip proxy
2. Dialplan routing resolves: app=rwi, context="ivr_bot"
3. SuperSip creates RwiApp for the call, holds it in ringing state
...
This section organizes common call scenarios for RWI as a reference for E2E testing and development.
...
---
**Status:** ✅ Shipped
**Source:** `docs/rwi.md`, `docs/rwi_case.md`
