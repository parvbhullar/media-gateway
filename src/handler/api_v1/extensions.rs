...
mod extensions {
    ...
    /// trim+filter chain (see `create_extension` above, ~line 302-308) is
    /// what turns that empty string into "unassigned" (`ActiveValue::NotSet`),
    /// not serde.
    #[test]
    fn test_org_id_serde_roundtrip_and_handler_normalization() {
        let no_org: CreateExtensionRequest =
            serde_json::from_str(r#"{"extension":"102"}"#).expect("deserialize");
        assert_eq!(no_org.org_id, None, "omitted org_id must deserialize to None");

        let empty_org: CreateExtensionRequest =
            serde_json::from_str(r#"{"extension":"102","org_id":""}"#).expect("deserialize");
        assert_eq!(
            empty_org.org_id,
            Some("".to_string()),
        ...
        // Mirrors the handler's own normalization (create_extension, ~line 302-308):
