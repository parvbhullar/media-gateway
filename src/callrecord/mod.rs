...
/// Where a call failure originated, so a CDR can attribute (for example) a SIP
/// 503 to our own SBC versus the upstream carrier leg. Derived from the SIP
/// `TerminatedReason` at hangup time; absent for a clean (non-failure) teardown.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum FailureSource {
    /// Generated by our SBC/proxy itself (overload, no route, registrar error,
    /// auth challenge) — "our side".
    Sbc,
    /// Returned by the upstream/callee leg — the carrier trunk on an outbound
    /// call (relayed 503/486/603, or no answer / timeout).
    Upstream,
...
impl FailureSource {
    /// Stable lowercase token for the CDR `failure_source` column. Pair only
    /// with [`from_db_str`](Self::from_db_str).
    ...
    /// Inverse of [`as_db_str`](Self::as_db_str); unknown tokens yield `None`.
